...
O executável é criado em `dist/Petronect Email Extractor v0.0.1.1.exe`. O usuário escolhe o destino da planilha; o log é salvo ao lado dela com o sufixo `_log.txt`. Falhas de inicialização são registradas em `%LOCALAPPDATA%\PetronectEmailExtractor\logs`.
...
```powershell
git tag -a v0.0.1.1 -m "Petronect Email Extractor v0.0.1.1"
